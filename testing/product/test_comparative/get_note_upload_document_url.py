...
def get_note_upload_document_url(
    access_token: str, company_id: str, product_id: str, file_name: str
):
    url = f"{environment.base_url}/product/{product_id}/test-comparison/note/upload-document-url?file_name={file_name}"
    headers = {
...
def test() -> None:
    ...
    product_test_comparison = get_note_upload_document_url(
        access_token, company_id, product_id, "test_document.jpg"
    )
