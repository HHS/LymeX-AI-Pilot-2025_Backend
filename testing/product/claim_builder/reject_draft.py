...
def reject_draft(access_token: str, company_id: str, product_id: str):
    url = f"{environment.base_url}/product/{product_id}/claim-builder/draft/reject"
    headers = {
        "Authorization": f"Bearer {access_token}",
        "Company-ID": company_id,
    }
    response = requests.post(url, headers=headers, json={"reject_message": "test"})
...
def test() -> None:
    ...
    try:
        result = reject_draft(access_token, company_id, product_id)
