...
def update_draft(access_token: str, company_id: str, product_id: str):
    url = f"{environment.base_url}/product/{product_id}/claim-builder/draft"
    headers = {
        "Authorization": f"Bearer {access_token}",
        "Company-ID": company_id,
    }
    response = requests.put(url, headers=headers, json={"content": "abcxys"})
...
def test() -> None:
    ...
    try:
        result = update_draft(access_token, company_id, product_id)
