...
def get_product_claim_builder_analyze_progress(
    access_token: str, company_id: str, product_id: str
):
    url = f"{environment.base_url}/product/{product_id}/claim-builder/analyze-progress"
    headers = {
...
def test() -> None:
    ...
    product_claim_builder = get_product_claim_builder_analyze_progress(
        access_token, company_id, product_id
    )
    print(f"Product Claim Builder: {json.dumps(product_claim_builder, indent=4)}")
