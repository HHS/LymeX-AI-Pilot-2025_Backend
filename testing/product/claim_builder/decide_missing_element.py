...
def decide_missing_element(access_token: str, company_id: str, product_id: str):
    url = f"{environment.base_url}/product/{product_id}/claim-builder/missing-element/decide"
    headers = {
        "Authorization": f"Bearer {access_token}",
        "Company-ID": company_id,
    }
    response = requests.post(url, headers=headers, json={"id": 0, "accepted": True})
...
def test() -> None:
    ...
    try:
        result = decide_missing_element(access_token, company_id, product_id)
