...
def get_analyze_progress(access_token: str, company_id: str, product_id: str):
    url = f"{environment.base_url}/product/{product_id}/profile/analyze-progress"
    headers = {
...
def test() -> None:
    ...
    product_profile = get_analyze_progress(access_token, company_id, product_id)
    print(f"Product Profile: {json.dumps(product_profile, indent=4)}")
