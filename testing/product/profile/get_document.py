...
def get_document(access_token: str, company_id: str, product_id: str):
    url = f"{environment.base_url}/product/{product_id}/profile/document"
    headers = {
...
def test() -> None:
    ...
    product_profile_document = get_document(access_token, company_id, product_id)
    print(
        f"Product Competitive Analysis Document: {json.dumps(product_profile_document, indent=4)}"
    )
