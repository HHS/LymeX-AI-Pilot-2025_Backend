...
def get_result(access_token: str, company_id: str, product_id: str):
    url = f"{environment.base_url}/product/{product_id}/competitive-analysis/result"
    headers = {
...
def get_first_analyze_result(access_token: str, company_id: str, product_id: str):
    results = get_result(access_token, company_id, product_id)
    if not results:
        raise ValueError("No competitive analysis results found.")
    # As 0 is self
    return results[1]
...
def test() -> None:
    ...
    result = get_result(access_token, company_id, product_id)
