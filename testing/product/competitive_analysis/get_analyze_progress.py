...
def get_product_competitive_analysis_analyze_progress(
    access_token: str, company_id: str, product_id: str
):
    url = f"{environment.base_url}/product/{product_id}/competitive-analysis/analyze-progress"
    headers = {
...
def test() -> None:
    ...
    product_competitive_analysis = get_product_competitive_analysis_analyze_progress(
        access_token, company_id, product_id
    )
    print(
        f"Product Competitive Analysis: {json.dumps(product_competitive_analysis, indent=4)}"
    )
