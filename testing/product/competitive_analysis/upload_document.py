...
def get_document_upload_url(access_token: str, company_id: str, product_id: str):
    file_name = test_file_path.split("/")[-1]
    url = f"{environment.base_url}/product/{product_id}/competitive-analysis/document/upload-url?file_name={file_name}&category=product"
    headers = {
...
def test() -> None:
    ...
    document_upload_url = get_document_upload_url(
        access_token,
        company_id,
        product_id,
    )
    print(f"document_upload_url: {document_upload_url}")
    upload_file(
        document_upload_url,
        test_file_path,
    )
