...
def get_company_folder(company: "Company") -> str:
    return f"company/{company.id}"


def get_company_logo_object_name(company: "Company") -> str:
    company_folder = get_company_folder(company)
    return f"{company_folder}/logo"


async def get_company_logo_url(company: "Company") -> str:
    logo_object_name = get_company_logo_object_name(company)
    logo_url = await generate_get_object_presigned_url(logo_object_name)
    return logo_url


async def get_update_company_logo_url(company: "Company") -> str:
    logo_object_name = get_company_logo_object_name(company)
