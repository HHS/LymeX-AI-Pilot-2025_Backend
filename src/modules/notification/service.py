...
async def get_user_notifications(
    ...
    notifications = (
        await Notification.find(query).sort(-Notification.created_at).to_list()
    )
...
async def get_notification_by_id(
    notification_id: str, user_id: str, company_id: str
) -> Optional[NotificationResponse]:
    """Get a specific notification by ID."""
    notification = await Notification.find_one(
        {"_id": notification_id, "user_id": user_id, "company_id": company_id}
    )
...
async def mark_notification_as_read(
    ...
    return await update_notification(
        notification_id, user_id, company_id, NotificationUpdate(is_read=True)
    )


async def mark_all_notifications_as_read(
    user_id: str, company_id: str, category: Optional[NotificationCategory] = None
) -> int:
