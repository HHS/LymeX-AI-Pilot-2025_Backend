...
async def analyze_test_comparison_task_async(
    ...
    test_comparison = TestComparison(
        ...
        identified_gaps=[
            IdentifiedGap(
                name="Mechanical Testing Gap",
                description="Additional fatigue testing needed to match comparator device standards",
            ),
            IdentifiedGap(
        ...
        suggested_adjustments=[
            SuggestedAdjustment(
                name="Expand Mechanical Testing Scope",
                description="Include cyclic loading tests and extended durability assessment",
            ),
            SuggestedAdjustment(
    ...
    await TestComparison.find_one(TestComparison.product_id == product_id).delete()
